...
is the principal way to make code run faster. In Julia, however, type declaration generally has no impact on performance 
because the compiler generally knows the types of all function arguments, 
local variables, and expressions. However, there are a few specific instances 
where declarations can be helpful.
